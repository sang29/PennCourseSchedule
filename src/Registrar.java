...
public class Registrar implements IRegistrar {
    ...
    public Collection<ICourse> parseSections(Map<String, String> subjects) {
        ...
        try {
            for (Entry<String, String> subject : subjects.entrySet()) {
                ...
                code = subject.getKey();
                Elements elements = timeTable.getElementsByTag("pre").get(0)
                        .getElementsByTag("p");
                ...
                while (s.hasNextLine()) {
                    String line;
                    do {line = s.nextLine();} while (line.isBlank());
    ...
    public Course parseSectionHelper(String str, String code, int courseNumber, Scanner scnr) {
        ...
        if (instructor.length() > 0) {
            ...
            if ((i = instructor.indexOf("/")) != -1) {
                instructor = instructor.replace("/" + instructor.charAt(i+1), "/" +Character.toUpperCase(instructor.charAt(i+1)));
            }
        }
        c.setInstructor(instructor);
        while (scnr.hasNextLine() && !(str.contains("MAX:") || str.contains("MAX W/"))) str = scnr.nextLine().trim();
    ...
    public void parseDuration(String s, Course c) {
    ...
    public Map<String, Map<Integer, ICourse>> buildCourseDirectory(
            Map<String, Map<Integer, String>> catalog) {
...
//        System.out.println(c.duration());
        ...
        while (s.hasNextLine()) {
            ...
            if (line.substring(0, code.length()).equals(code)) {
                ...
                while (s.hasNextLine() && !(line = s.nextLine()).isBlank()) {
                    if (isNumeric(line.substring(1, 4))) {
                        ICourse c = r.parseSectionHelper(line, code, courseNumber, s);
                        System.out.println(c);
